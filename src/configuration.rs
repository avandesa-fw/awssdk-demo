...
pub fn load_app_config() -> Result<BridgeServiceConfig> {
    ...
    let environment_file = configuration_directory.join(format!("{}.toml", environment.as_str()));
    ...
    settings
        .try_deserialize::<BridgeServiceConfig>()
        .map_err(Into::into)
}
